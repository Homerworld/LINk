...
const { getDoc, addDoc, updateDoc, queryDocs, increment, runTransaction } = require('../config/firebase');
...
// Called by the app after the Paystack redirect. Confirms server-side instead
// of relying only on the webhook (which can be delayed or dropped).
exports.verifyPayment = async (req, res) => {
  try {
    const { reference } = req.params;
    const jobs = await queryDocs('jobs', [['paystackReference', '==', reference]], null, 1);
    if (jobs.length === 0) return fail(res, 'Payment reference not found', 404);
    const job = jobs[0];
    // Only the paying customer can verify their own payment.
    if (job.customerId !== req.user.userId) return fail(res, 'Not authorized', 403);
    if (job.paymentVerified) return ok(res, { status: 'active', jobId: job.id }, 'Payment already confirmed');

    const key = process.env.PAYSTACK_SECRET_KEY;
    if (!key || key.includes('xxxx')) {
      return fail(res, 'Payment not yet confirmed', 402);
    }

    const pRes = await PAYSTACK.get(`/transaction/verify/${encodeURIComponent(reference)}`);
    const status = pRes.data?.data?.status;
    if (status === 'success') {
      await activateJob(reference);
      return ok(res, { status: 'active', jobId: job.id }, 'Payment confirmed');
    }
    return fail(res, `Payment ${status || 'not completed'}`, 402);
  } catch (err) {
    logger.error('verifyPayment: ' + err.message);
    return fail(res, 'Could not verify payment');
  }
};
...
const activateJob = async (reference) => {
  ...
  await runTransaction(async (tx, { txGet, txUpdate }) => {
    ...
    if (job.paymentVerified) return; // idempotent — webhook + verify can both fire
    ...
    txUpdate('users', job.vendorId, { escrowBalance: increment(job.vendorPayout) });
  });
  logger.info(`Job activated: ${jobId}`);
};
